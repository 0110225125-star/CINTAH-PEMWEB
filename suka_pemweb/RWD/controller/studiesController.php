...
switch ($tombol) {
    ...
    case 'hapus':
        // Tangkap 'id' yang dikirim dari hidden input di studies_list.php
        $id_hapus = $_POST['id'] ?? ''; 
        if (!empty($id_hapus)) {
            $obj_studies = new Studies(); // Pastikan objek dibuat
            $obj_studies->hapus($id_hapus);
        }
        break;
}

// Redirect balik ke halaman list agar tidak berhenti di halaman putih
